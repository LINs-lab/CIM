import os
import random
import data.utils.serialize as serialize
import numpy as np
import lmdb
import time
import pickle
...
class ImageFolder(torchvision.datasets.ImageFolder):
    ...
    def __init__(self,
        ...
        self.mem = mem
        self.image_paths = [
        ]  # List to store file paths for all sampled images
...
class LMDBPTClass(torch.utils.data.Dataset):
    ...
    def _prepare_cache(self):
        ...
        if os.path.isfile(cache_file):
            ...
            with self.env.begin(write=False) as txn:
                self.keys = [
                    key for key, _ in txn.cursor() if key != b"__keys__"
                ]
...
class LMDBPT(torch.utils.data.Dataset):
    ...
    def _build_indices(self):
        self.from_to_indices = enumerate(
            zip(self.indices[:-1], self.indices[1:]))
    ...
    def __getitem__(self, index, apply_transform=True):
        ...
        image, target = self.dbs[block_index].__getitem__(
            item_index, apply_transform)
        return image, target
...
def load_dataset(
    ...
    # if ipc == -1 and classes == [] and dataset == "imagenet-1k":
    ...
    if dataset == "imagenet-1k-lmdb":
        ...
        dataset = LMDBPTClass(root=dataset_dir, transform=transform)
...
def load_normalize(dataset="cifar10"):
    ...
    return transforms.Normalize(mean=data_stats[dataset]["mean"],
                                std=data_stats[dataset]["std"])
