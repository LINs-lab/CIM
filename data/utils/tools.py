...
def dist_l2(data, target):
    dist = ((data**2).sum(-1).unsqueeze(1) + (target**2).sum(-1).unsqueeze(0) -
...
class TimeStamp:
    ...
    def __init__(self, save_dir, print_log=True):
        self.prev = time.time()
        self.times = {}

        if print_log == True:
            self.logger = Logger(save_dir, "cost_log")
        else:
            self.logger = print
    ...
    def flush(self):
        ...
        self.logger(f"Peak Memory of GPU: {self.peak_memory_gpu:.2f} GB")

    def stamp(self, name=""):

        spent = time.time() - self.prev
        # self.logger(f"{name}: {spent:.4f}s")
...
def save_img(save_dir, img, max_num=100, size=64, nrow=10):
    ...
    save_image(img.cpu(), save_dir, nrow=nrow, normalize=True)
