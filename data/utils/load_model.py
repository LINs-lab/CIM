    ...
    def pruning_classifier(model=None, classes=[]):
        try:
            model_named_parameters = [
                name for name, x in model.named_parameters()
            ]
            for name, x in model.named_parameters():
                if (name == model_named_parameters[-1]
        ...
        if dataset in [
            ...
            if custom_model != "":
                checkpoint = torch.load(
                    f"./data/pretrain_models/{custom_model}.pth",
                    map_location="cpu")
                ...
                model.load_state_dict(checkpoint["model"])
        elif dataset in ["imagenet-1k", "imagenet-1k-lmdb"]:
...
def get_features(model=None, image=None, hook_indices=[-1]):
    if isinstance(model, ConvNet):
        ...
        features, output = model.get_feature(image,
                                             idx_from=model.depth,
                                             idx_to=-1,
                                             return_prob=False,
                                             return_logit=True)
        layer_outputs = []
